...
describe("Gratuity Contract", () => {
  ...
  describe("Transactions", async () => {
    ...
    it("should succeed if the owner tries to withdraw funds", async () => {
      ...
      let beforeContractBalance = await hre.ethers.provider.getBalance(
        contract.address
      );
      console.log(
        "Contract balance before:",
        hre.ethers.utils.formatEther(beforeContractBalance)
      );

      let beforeOwnerBalance = await owner.getBalance();
      console.log(
        "Balance of owner before withdrawal:",
        hre.ethers.utils.formatEther(beforeOwnerBalance)
      );
      ...
      console.log(
        ...
        hre.ethers.utils.formatEther(afterOwnerBalance)
      );
      ...
      expect(hre.ethers.utils.formatEther(afterContractBalance)).to.equal(
        "0.0"
      );
    ...
    it("should fail if an account that is not the owner tries to withdraw funds", async () => {
      await expect(
        contract.connect(randomAccount).withdraw()
    ...
    it("should fail if an account that is not the owner tries to transfer funds", async () => {
      await expect(
        contract
          .connect(randomAccount)
          .transfer(owner.address, hre.ethers.utils.parseUnits(".25", "ether"))
      ).to.be.revertedWith("caller is not the owner");
    });
  });
});
